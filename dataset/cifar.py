...
from dataset.utils import download_url, check_integrity, noisify
...
class CIFAR10(data.Dataset):
    ...
    def __init__(self, root, train=True,
                 ...
                 download=False,
                 noise_type=None, noise_rate=0.2, random_state=0, redux=None):
        ...
        if self.train:
            ...
            if redux is not None:
                assert len(self.train_data) >= redux
                index = np.random.choice(len(self.train_data), redux, replace=False)
            ...
            if noise_type != 'clean':
                ...
                self.labels = self.train_labels.squeeze()
            else:
            ...
            if redux is not None:
                assert len(self.test_labels) >= redux
                index = np.random.choice(len(self.test_data), redux, replace=False)
